...
async function getSongs(folder) {
    ...
    try {
        ...
        let res = await fetch(`${CLOUDINARY_BASE}/raw/upload/songs/${encodedFolder}/info.json`);
...
async function displayAlbums() {
    ...
    for (let folder of folders) {
        try {
            const encodedFolder = encodeURIComponent(folder);
            const metaRes = await fetch(`${CLOUDINARY_BASE}/raw/upload/songs/${encodedFolder}/info.json`);
...
async function main() {
    await getSongs("BestOnes"); // updated name (no apostrophe)
    playMusic(songs[0], true);
    await displayAlbums();
