...
const META_URL = "https://res.cloudinary.com/dd4repvz4/raw/upload/v1751231016/meta_aaxemd.json";
...
async function getSongs(folder) {
    currFolder = folder;
    try {
        const infoURL = metaMapping[folder];
        if (!infoURL) throw new Error("No info.json URL for folder");
...
async function displayAlbums() {
    ...
    try {
        ...
        metaMapping = await metaRes.json();

        for (const folder in metaMapping) {
            try {
                const infoRes = await fetch(metaMapping[folder]);
                const info = await infoRes.json();
                const coverUrl = `${CLOUDINARY_BASE}/image/upload/songs/${encodeURIComponent(folder)}/cover.jpg`;
                ...
                console.warn(`⚠️ Skipping folder ${folder}`, err);
            }
        }
